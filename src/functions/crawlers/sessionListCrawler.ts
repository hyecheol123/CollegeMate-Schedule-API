...
export default async function sessionListCrawler(
  ...
  courseId: string
    ...
    await fetch(
        `https://public.enroll.wisc.edu/api/search/v1/enrollmentPackages/${termCode}/${subjectCode}/${courseId}`
      )
          ...
          data.forEach((session: any) => {
            ...
            session.sections.forEach((section: any) => {
              ...
              section.classMeetings.forEach((meeting: any) => {
                ...
                meetingList.push({
                  buildingName: !meeting.building ? undefined : meeting.building.buildingName,
                  room: meeting.room,
