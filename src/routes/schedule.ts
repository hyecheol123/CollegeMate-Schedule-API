 ...
 * Limited to course search and available semesters only
 *
...
// COMMENTED OUT: POST: /schedule - Create new schedule with full authentication
...
// COMMENTED OUT: DELETE: /schedule/:scheduleId - Delete schedule with authentication
...
//     );
//
//     // Validate scheduleId and user ownership
...
// GET: /schedule/available-semesters - Active endpoint with origin validation only
scheduleRouter.get('/available-semesters', async (req, res, next) => {
  const dbClient: Cosmos.Database = req.app.locals.dbClient;
  try {
    // Check Origin header validation
    if (req.header('Origin') !== req.app.get('webpageOrigin')) {
      throw new ForbiddenError();
    }
...
// COMMENTED OUT: DELETE: /schedule/:scheduleId/event/:eventId - Remove event from schedule with auth
...
// GET: /schedule/course - Active endpoint with origin validation only
...
// COMMENTED OUT: POST: /schedule/course-list/:termCode/update - Admin course data update with server token
...
// COMMENTED OUT: GET: /schedule/{base64Email}/list - Get user's schedule list with JWT auth and friend validation
...
// COMMENTED OUT: GET: /schedule/{scheduleId} - Get schedule details with JWT auth and friend validation
